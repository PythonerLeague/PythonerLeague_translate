...
class WhisperTranslate(object):
    def __init__(self, model_name='large', language='Chinese', compute_engine='cpu',
                 model_path_root=os.path.dirname(os.path.abspath(__file__)), mes_q=queue.Queue()):
        ...
        if self.model_name != 'large':
            warnings.warn("警告，选择的模型不是large，如果语音跟要翻译成的语言不一致，large模型效果最好，但是速度会最慢，建议选择large模型，如果一致，选择小模型即可")
            self.mes_q.put("警告，选择的模型不是large，如果语音跟要翻译成的语言不一致，large模型效果最好，但是速度会最慢，建议选择large模型，如果一致，选择小模型即可")
            ...
            with open(save_path + '/{}.srt'.format(file_name), 'w', encoding='utf-8') as f:
                for i in result["segments"]:
                    ...
                    if self.language == 'chinese':
                        ...
                        srt_str += '{} --> {}:{}\n'.format(time.strftime('%H:%M:%S,%d', time.gmtime(i['start'])),
                                                           time.strftime('%H:%M:%S,%d', time.gmtime(i['end'])),
                                                           Converter('zh-hans').convert(i['text']))
                    else:
                        f.write('{}\n\n'.format(i['text']))
                        srt_str += '{} --> {}:{}\n'.format(time.strftime('%H:%M:%S,%d', time.gmtime(i['start'])),
                                                           time.strftime('%H:%M:%S,%d', time.gmtime(i['end'])),
    ...
    def add_subtitle(self, translate_file, save_path):
        try:
            ...
            cmd = "ffmpeg -i {} -vf subtitles=\\'{}\\' -y {}_add_subtitle.{}".format(translate_file,
                                                                                     save_path + '/{}.srt'.format(
                                                                                         file_name),
                                                                                     save_path + '/{}'.format(
                                                                                         file_name), file_type)
